function data = tldc_algo_dec(channel,message)
%Channel length
...
for i = 1:message_length/16
    for j = 1:16
        Encrypted_Vector_temp(i,j) = message((j-1)*(message_length/16)+i);
    end
end

for i = 1:message_length/16
    Encrypted_Vector(i) = b2d(Encrypted_Vector_temp(i,1:end));
end
...
data = mod(round(Decrypted_Vector),2);
